    ...
    fn from(field : &Field, num : usize) -> Vec<Grid> {
        ...
        for _ in 0..num {
            loop {
                ...
                if field[y as usize][x as usize] != Square::Obstacle || field[y as usize][x as usize] != Square::Unknown { 
                    continue;
                }
...
impl Wrapper for GridCloningWrapper {
...
fn test_grids_create() {
    let field = Field {
        ...
        booster_field: vec![vec![]],
        rest_booster_cnts: vec![],
        rest_surface_cnt: 0,
    };
    let partition = Partition { x: 2, y: 3 };
    let grids = Grids::create(&field, &partition);
    println!("{:?}", grids);
}

// #[test]
// fn test_grid_cloning_two_cloning() {
//     // .X...
//     // ..#..
//     // C.#..
//     // ..#..
...
//     let mut wrapper = GridCloningWrapper::new(&task, &vec![], 1 << 30);
